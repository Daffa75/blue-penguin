            ...
            @if($getState() != null)
                @foreach($getState() as $supervisor)
                    <div x-data x-tooltip.raw="{{ $supervisor->name }}" class="relative transition duration-300 ease-in-out hover:scale-150 hover:z-50">
                        <img class=" w-10 h-10 max-w-none object-cover object-center rounded-full"
                             src="
                         @if(!$supervisor->image_url)
                         {{ $getDefaultAvatar() }}
                         @elseif(!empty(parse_url($supervisor->image_url)['scheme']))
                         {{ $supervisor->image_url }}
                         @elseif(empty(parse_url($supervisor->image_url)['scheme']))
                         {{ 'https://eng.unhas.ac.id/siminformatika'.$supervisor->image_url }}
                         @else
                         {{ $getDefaultAvatar() }}
                         @endif
                         " alt="">
